...
1. **Context summarization implementation detail (D-22)**
   - What we know: Edge Function triggers summarization when `message_count > threshold`. Summary replaces older messages in the system prompt.
   - What's unclear: Should the summary be generated by the Edge Function calling GPT-4o mini with a dedicated summarization prompt, or should the iOS client maintain a local summary? Server-side is architecturally cleaner (keeps logic out of client).
   - Recommendation: Implement server-side. When `message_count > 50` (suggested threshold — Claude's discretion), the Edge Function makes a preliminary summarization call before the main chat call. The summary is ephemeral (not stored); it's rebuilt each request from the message archive. This adds latency on summarization-triggered requests but keeps the client simple.

2. **Plan modification scope detection**
   - What we know: The coach proposes modifications inline (D-05). The Edge Function must detect when a conversational turn is requesting a plan change vs. general Q&A.
   - What's unclear: How does the Edge Function reliably classify the intent? Options: (a) two-pass — conversational response first, then intent classification; (b) structured output with `action` field on every response; (c) keyword heuristics in the system prompt.
   - Recommendation: Add explicit intent classification to the system prompt: "When the user asks to change their plan, always include a JSON block at the end of your response in format `[ACTION]{...}` before responding naturally." This makes intent detection reliable without a second API call.

3. **Plan modification partial vs. full plan return**
   - What we know: D-07 supports single exercise swap, multi-exercise within a day, and full regeneration. D-09 calls GPT-4o with Structured Outputs.
   - What's unclear: Should the modification return the full plan (simpler client merge) or only the changed days (more efficient)?
   - Recommendation: Return the full plan using `planSchema`. The iOS client replaces `CDWorkoutPlan.rawJSON` and re-parses `CDWorkoutDay`/`CDPlannedExercise`. Simpler than partial merging; plan JSON is small (<5KB). Optimize to partial if needed in Phase 8.
...
|--------|----------|-----------|-------------------|-------------|
| CHAT-01 | CoachViewModel.sendMessage() creates pending message and starts streaming state | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachViewModelTests` | ❌ Wave 0 |
| CHAT-01 | CoachSSEClient parses `.token` events from SSE stream | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachSSEClientTests` | ❌ Wave 0 |
| CHAT-01 | Offline state disables send and shows banner | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachViewModelTests/testOfflineDisablesSend` | ❌ Wave 0 |
| CHAT-02 | Action envelope parsed correctly (`"modify_plan"` triggers card state) | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachViewModelTests/testModifyPlanActionTriggersCard` | ❌ Wave 0 |
| CHAT-02 | [Confirm] tap sets planModificationState = "confirmed" in CoreData | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachViewModelTests/testConfirmPlanModification` | ❌ Wave 0 |
| CHAT-02 | [Dismiss] tap sets planModificationState = "dismissed" | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachViewModelTests/testDismissPlanModification` | ❌ Wave 0 |
| CHAT-03 | System prompt payload includes profile + plan + last 3 session summaries | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachViewModelTests/testSystemPromptPayloadContents` | ❌ Wave 0 |
| CHAT-03 | Message payload caps message_history at last 20 messages | unit | `xcodebuild test -only-testing:WorkoutAppTests/CoachViewModelTests/testMessageHistoryCap` | ❌ Wave 0 |
